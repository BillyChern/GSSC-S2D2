...
from .point_encoder import voxel_3d_generator, voxelization
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if use_lidar:
            ...
            if sparse_conv:
                self.lidar_embedding = SparseSequential(
                    SparseConv3d(1, model_channels, 3, padding=1, stride=downsample_resolution, indice_key=None),
                    SiLU(),
                    SubMConv3d(model_channels, model_channels, 3, padding=1, indice_key=None, bias=False),
                    SiLU(),
                    normalization(model_channels),
                )

            elif not use_point_encoder:
        ...
        if use_multiclass_bev:
            self.multiclass_bev_embed = nn.Sequential(
                conv_nd(3, num_classes, model_channels, 3, padding=1, stride=downsample_resolution),
                SiLU(), 
        ...
        use_bev = use_bev or use_bev_vol or use_multiclass_bev
    ...
    def forward(self, x, timesteps, x_paths=None, y=None, bev=None, classifier_free=None, condition_scale=None, bev_mask=None, pc_config=None):
        """
        ...
        if self.use_lidar:
            if self.sparse_conv:
                ...
                '''print(lidar_features.shape, lidar_features.type())
                print(lidar_coords.shape, lidar_coords.type())
                print(spatial_shape, batch_size)
                assert False'''
                ...
                print(lidar_features.device, lidar_coords.device, spatial_shape)
                print(lidar_features.type(), lidar_coords.type())'''
                lidar_emb = self.lidar_embedding(input_sp_lidar)
                
            elif self.use_point_encoder:
                coors_range_xyz = pc_config['coors_range_xyz'][0].squeeze()
                spatial_shape = pc_config['spatial_shape'][0].squeeze()
                pc = pc_config['pointcloud_data'].squeeze()
                batch_idx = pc_config['batch_idx'].squeeze()
                self.voxelizer.set_pc_config(coors_range_xyz, spatial_shape)
                self.voxel_3d_generator.set_pc_config(coors_range_xyz, spatial_shape, x.shape[0])
                
                points, bxyz_indx, unq, unq_inv = self.voxelizer(pc, batch_idx)
                lidar_emb = self.voxel_3d_generator(points, bxyz_indx, unq, unq_inv)
        ...
        if self.use_multiclass_bev:
            bev = bev.unsqueeze(2).expand(-1,-1,x.shape[-3],-1,-1)
            bev_emb = self.multiclass_bev_embed(bev)
