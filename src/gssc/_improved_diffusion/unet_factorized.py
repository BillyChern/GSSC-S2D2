"""Vendored improved-diffusion 3D U-Net, factorized-convolution variant.

Research reference only: the released S2D2 denoiser is
``src/gssc/models/s2d2_unet.py``, and no shipped config reaches this module.
...
class TimestepBlock(nn.Module):
    ...
    @abstractmethod
    def forward(self, x, emb, lidar_emb=None):
        """
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2):
        ...
        if use_conv:
            ...
            self.conv = factorized_conv_nd(dims, channels, channels)
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2):
        ...
        if use_conv:
            ...
            self.op = factorized_conv_nd(dims, channels, channels, True)
        else:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            SiLU(),
            factorized_conv_nd(dims, channels, self.out_channels),
        )
        ...
        if use_lidar:
            if sparse_conv:
                ...
                self.lidar_emb_layers = nn.Sequential(
                    ...
                    factorized_conv_nd(dims, model_channels, self.out_channels),
                    SiLU(),
                    normalization(self.out_channels),
                )
        ...
        self.out_layers = nn.Sequential(
            ...
            zero_module(
                ...
                factorized_conv_nd(dims, self.out_channels, self.out_channels),
            ),
        )
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = factorized_conv_nd(dims, channels, self.out_channels)
        else:
    ...
    def forward(self, x, emb, lidar_emb=None):
        ...
        """
        return checkpoint(
            self._forward, (x, emb, lidar_emb), self.parameters(), self.use_checkpoint
        )

    def _forward(self, x, emb, lidar_emb=None):
        h = self.in_layers(x)
        emb_out = self.emb_layers(emb).type(h.dtype)
        #print(h.shape, lidar_emb.shape, self.lidar_emb_layers(lidar_emb).shape)
        if lidar_emb != None:
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if use_lidar:
            ...
            if sparse_conv:
                ...
                self.lidar_embedding = nn.Embedding(20, model_channels)
                self.sparse_shape = [32,256,256]
            else:
        ...
        self.input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    ...
                    factorized_conv_nd(dims, in_channels, model_channels),
                )
            ]
        )
        ...
        self.out = nn.Sequential(
            ...
            zero_module(factorized_conv_nd(dims, model_channels, out_channels)),
        )
    ...
    def forward(self, x, timesteps, x_paths=None, y=None, classifier_free=None, condition_scale=None):
        """
        ...
        if y != None:
            ...
            if self.sparse_conv:
                ...
                voxel_batches = th.ones(batch_size,1,32,256,256, device=x.device) * th.arange(0,batch_size,1,device=x.device).view(-1,1,1,1,1)
                ...
                lidar_features = self.lidar_embedding(lidar_voxels.clone().detach())
        ...
        for module in self.input_blocks:
            h = module(h, emb, lidar_emb)
            hs.append(h)
        h = self.middle_block(h, emb, lidar_emb)
        for module in self.output_blocks:
            h_ = hs.pop()
            cat_in = th.cat([h, h_], dim=1)
            h = module(cat_in, emb, lidar_emb)
        h = h.type(x.dtype)
        out = self.out(h)
        #if self.binary:
        #    out = F.threshold(0.5)
