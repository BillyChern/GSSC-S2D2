    ...
    def __init__(
        ...
        if use_lidar:
            if sparse_conv:
                ...
                while mult > 1:
                    mult /= 2
                    emb_layers.append(SparseMaxPool3d(kernel_size=2, stride=2))
                emb_layers = emb_layers + [
                    #SparseConv3d(model_channels, self.out_channels, 3, stride=mult, padding=1, indice_key=None),
                    SubMConv3d(model_channels, self.out_channels, 3, padding=1, indice_key=None),
                    SiLU(),
                    SubMConv3d(self.out_channels, self.out_channels, 3, padding=1, indice_key=None),
                    SiLU(),
                    normalization(self.out_channels),
                ]
                self.lidar_emb_layers = SparseSequential(*emb_layers)
            else:
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, x_paths=None, y=None, bev=None, classifier_free=None, condition_scale=None, bev_mask=None):
        """
