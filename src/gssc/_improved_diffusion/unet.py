...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if encoder_config is not None:
            self.voxel_encoder = Encoder(encoder_config['num_labels'], encoder_config['num_res_blocks'], encoder_config['channels'])
            self.voxel_encoder.load_state_dict(th.load(encoder_config['state_dict_path'], map_location='cpu'), strict=False)
        ...
        if use_lidar:
            ...
            if sparse_conv:
                self.lidar_embed = SparseSequential(
                ...
                if analog_bits:
                    #self.lidar_embedding = SparseConv3d(5, model_channels, 3, padding=1, stride=downsample_resolution, bias=False, indice_key=None)
                    self.lidar_embedding = SparseConv3d(1, model_channels, 3, padding=1, stride=downsample_resolution, bias=False, indice_key=None)
                elif one_hot:
                    self.lidar_embedding = SparseConv3d(20, model_channels, 3, padding=1, stride=downsample_resolution, bias=False, indice_key=None)
                else:
                    self.lidar_embedding = SparseConv3d(19, model_channels, 3, padding=1, stride=downsample_resolution, bias=False, indice_key=None)

                #self.sparse_shape = [32/downsample_resolution,256/downsample_resolution,256/downsample_resolution]
            else:
        ...
        if use_bev_vol:
            ...
            self.bev_embed = SparseSequential(
                conv_nd(3, in_channels, model_channels, 3, padding=1, stride=downsample_resolution),
                SiLU(), 
                conv_nd(3, model_channels, model_channels, 3, padding=1),
                SiLU(),
                normalization(model_channels),
    ...
    def forward(self, x, timesteps, x_paths=None, y=None, bev=None, classifier_free=None, condition_scale=None):
        ...
        :param bev: an [N x H x W] Tensor of conditioning bird's eye view.
        ...
        #    th.save(x.clone().cpu(), x_paths)
        
        #for path in x_paths:
        #    assert path != "None"
        '''import os
        print(timesteps)
        sample = (x > 0).type(th.float)
        value = (sample[:,0] * 16 + sample[:,1] * 8 + sample[:,2] * 4 + sample[:,3] * 2 + sample[:,4]).clone()
        out = value[0].permute(2,1,0).clone().cpu().numpy()
        np.save("vis_an/000001.label",out.astype(np.int16).reshape(-1))
        os.system("mv vis_an/000001.label.npy vis_an/000001.label")
        sample = (y > 0).type(th.float)
        value = (sample[:,0] * 16 + sample[:,1] * 8 + sample[:,2] * 4 + sample[:,3] * 2 + sample[:,4]).clone()
        out = value[0].permute(2,1,0).clone().cpu().numpy()
        np.save("vis_an/000002.label",out.astype(np.int16).reshape(-1))
        os.system("mv vis_an/000002.label.npy vis_an/000002.label")
        assert False'''
        hs = []
        ...
        uncond_flag = 0
        if y != None:
            #y = self.lidar_pooling(y)
            #print(lidar_embedding(y, self.model_channels).shape)
            if self.sparse_conv:
                ...
                lidar_mask = y != 0
                lidar_mask = th.sum(lidar_mask, dim=1) != 0
                lidar_voxels = y.permute(0,2,3,4,1)[lidar_mask].clone()
                #print(lidar_mask.shape, voxel_coords.shape, th.unique(lidar_mask), lidar_voxels.shape)
                #print(voxel_coords.permute(0,2,3,4,1).shape, lidar_mask.unsqueeze(-1).shape)
                lidar_coords = voxel_coords.permute(0,2,3,4,1)[lidar_mask].clone()
                lidar_features = lidar_voxels
                ...
                #print(input_sp_lidar.dense().shape, lidar_voxels.shape, (y==input_sp_lidar.dense()).all())
                #print(input_sp_lidar.dense().shape)
                input_sp_lidar = self.lidar_embedding(input_sp_lidar)
                #print(input_sp_lidar.dense().shape)
                #print(input_sp_lidar.dense().shape, lidar_voxels.shape)
                '''print(lidar_coords.shape)
                print(th.max(lidar_coords[:,0]), th.min(lidar_coords[:,0]))
                print(th.max(lidar_coords[:,1]), th.min(lidar_coords[:,1]))
                print(th.max(lidar_coords[:,2]), th.min(lidar_coords[:,2]))
                print(th.max(lidar_coords[:,3]), th.min(lidar_coords[:,3]))
                print(self.sparse_shape)'''
                lidar_emb = self.lidar_embed(input_sp_lidar)
                
            else:
                lidar_emb = self.lidar_embed(lidar_embedding(y, self.model_channels))
            #print(y.shape, lidar_emb.shape,emb.shape)
        
        if uncond_flag:
            if self.use_bev:
                target = th.ones(x.shape[0],x.shape[3],x.shape[4]).to(x.device) * 11
                bev = th.zeros(x.shape[0],4,x.shape[3],x.shape[4]).to(x.device)
                #print(target.shape, batch_bev_bits.shape, batch_bev_bits[:,bits_cnt-1-1:bits_cnt-1].shape)
                for k in range(0,4):
                    bits_k = th.fmod(target, 2)
                    bev[:,4-1-k:4-k] = bits_k
                    target = target.div(2, rounding_mode="trunc")
            
            elif self.use_bev_vol:
                bev = th.ones_like(x) * 11

        if self.use_bev:
            bev_index = bev[:,0] * 8 + bev[:,1] * 4 + bev[:,2] * 2 + bev[:,3]
            ...
            bev_embed = self.bev_embed_3D(bev_volume)
            
        if self.use_bev_vol:
            bev_embed = self.bev_embed(bev)
        ...
        for module in self.input_blocks:
            h = module(h, emb, lidar_emb, bev_embed, uncond_flag)
            hs.append(h)
        h = self.middle_block(h, emb, lidar_emb, bev_embed, uncond_flag)
        for module in self.output_blocks:
            h_ = hs.pop()
            cat_in = th.cat([h, h_], dim=1)
            h = module(cat_in, emb, lidar_emb, bev_embed, uncond_flag)
        h = h.type(x.dtype)
        out = self.out(h)
        #if self.binary:
        #    out = F.threshold(0.5)
        #    out = (out > 0.5).type(th.float)
        return out
