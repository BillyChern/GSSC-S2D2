...
class LaserScan:
  ...
  def do_range_projection(self):
    ...
    #print(np.max(self.points[:,0]), np.max(self.points[:,1]), np.max(self.points[:,2]))
    #print(np.min(self.points[:,0]), np.min(self.points[:,1]), np.min(self.points[:,2]))
    # get depth of all points
    x_max = self.points[:,0] <= 51.2
    x_min = self.points[:,0] >= 0
    y_max = self.points[:,1] <= 25.6
    y_min = self.points[:,1] >= -25.6
    z_max = self.points[:,2] <= 4.4
    z_min = self.points[:,2] >= -2.0
    
    mask = x_max & x_min & y_max & y_min & z_max & z_min
    self.points = self.points[mask]
    
    depth = np.linalg.norm(self.points, 2, axis=1)
    #print(depth)
    #print(depth.shape, self.points.shape)
    #print(np.max(self.points[:,0]), np.max(self.points[:,1]), np.max(self.points[:,2]))
    #print(np.min(self.points[:,0]), np.min(self.points[:,1]), np.min(self.points[:,2]))
