...
[![SemanticKITTI](https://img.shields.io/badge/SemanticKITTI%20test-38.8%20mIoU%20%28N%3D1%2C%20no%20TTA%2C%20paper%20Tab.%20I%29-orange.svg)](https://www.codabench.org/competitions/13814/)

**SemanticKITTI hidden test: 38.8 % mIoU** at one step with no test-time augmentation — to our knowledge the best *causal, single-sweep, single-sample* result on the leaderboard to date, +2.1 pp over the previous best published score under that restriction (SCPNet, 36.7) — rising to **39.2 % mIoU** with four correction steps and *D*<sub>4</sub> TTA, a row the single-sample predicate excludes and which therefore carries no TALoS margin. Our public row on [Codabench](https://www.codabench.org/competitions/13814/#/results-tab) is the *D*<sub>4</sub>-TTA entry, which the results tab lists at 39.2 mIoU / 59.0 IoU<sub>cmpl</sub>; the 38.8 % *N* = 1 figure is the paper's hidden-test number (main Tab. I) and is not a row that can be looked up there. Treat the link as our leaderboard presence, not as the evidence for 38.8.
...
| **S²D² (Ours, *D*<sub>4</sub> TTA)** | **39.2** | 59.0 | **+2.5** | *N* = 4 with an 8-view ensemble. EXCLUDED from the paper's single-sample predicate, so it carries no TALoS margin. This is the row publicly displayed on the leaderboard results tab (39.2 / 59.0) — the only row of ours that is |

<sub><b>Source.</b> Every mIoU and IoU<sub>cmpl</sub> cell above — baseline rows and both S²D² rows — is paper-reported, in the paper's main results table (Tab. I); the Δ column is those values differenced against SCPNet's 36.7. The public SemanticKITTI SSC leaderboard is <i>not</i> the source: the only row of ours on its results tab is the <i>D</i><sub>4</sub>-TTA entry (39.2 / 59.0), so the <i>N</i>=1 row's 38.8 / 58.9 cannot be looked up there.</sub>
...
<sub>Two seq-08 frames where S²D² recovers a rare class the base SOTA misses entirely. Left → right: JS3C-Net, SCPNet, TALoS, S²D² (ours, <i>N</i>=4), Ground Truth. Source: paper Fig. 6.</sub>
