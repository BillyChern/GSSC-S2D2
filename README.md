...
> [!TIP]
> **In a hurry?** Skip to [Quick start](#quick-start-reproduce-3854--val-in-three-commands) for the 3-command reproduction recipe of the headline 38.54 % val mIoU, on a single H100 — once the SemanticKITTI voxel labels *and* the SCPNet base predictions are on disk. The voxel tree needs registration at [semantic-kitti.org](http://www.semantic-kitti.org/) and `scripts/download_assets.py` does **not** fetch it (see [docs/DATASET.md](docs/DATASET.md)); without it the evaluator finds an empty frame list and writes no labels. Budget **≥ 7 min** of GPU for the correction pass alone (107 ms × 4,071 val frames), plus the 8.4 GiB of prediction reads, the label writes and the CPU scorer on top. No end-to-end wall-clock is published for this recipe.
...
* **2026-08-25** — Release **v2.4.0**: **the published reproduction command was silently evaluating a subset, and now is not**. `scripts/download_assets.py` fetches and unpacks the released `.tar.zst` archives instead of reading a per-frame tree on the Hub. Several directories of the released corpus — 609,349 files across the three prediction trees and the object bank — hold more than the **10,000 files Hugging Face serves from one directory**, so `scpnet_predictions/08/` truncated at 10,000 files covering **3,334 of sequence 08's 4,071 val frames** — the headline `--include 'scpnet_predictions/08/*'` fetch returned an incomplete val set, exited 0 and reported no error. Re-measured after the fix against the live repo: **12,213 files, 4,071 distinct frame ids**, byte-identical to the reference tree. `--include` keeps its meaning and gets cheaper (val 08 is now **one 703 MiB / 737 MB archive** rather than 9.07 GB of per-frame transfers), sizes throughout the docs now separate **download** from **on-disk**, and `zstandard>=0.22` is a dependency so `uv sync` alone is enough.
* **2026-08-23** — Release **v2.3.8**, re-cut. The project is now **MIT**-licensed, with third-party attribution collected in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md). Adds `configs/train/pyramid_{s2,s3}.yaml` and the dispatcher fix that makes them runnable, plus `scripts/prepare_256_data.py` and `scripts/prepare_multi_frame_data.py`, which five code sites already pointed at. Corrects the JS3C-Net protocol attribution throughout: **26.7 %** is the derived-BEV row under the paper's internal evaluator and **24.3 %** is the same run under the official `semantic-kitti-api` — re-measured at **24.32 %** over all 4,071 val frames of seq 08. The commit history was rewritten ahead of first publication so that author metadata carries only the contact addresses the paper lists; every tree hash is unchanged.
...
# 2. Pull pretrained checkpoint + the SCPNet predictions step 3 reads (val seq 08)
...
# → data/scpnet_predictions/08/   12,213 files = all 4,071 val frames, 8.4 GiB / 9.1 GB on disk
#    The predictions ship as zstd archives; --include picks the ONE that can hold seq 08,
#    so this transfers 703 MiB / 737 MB and unpacks only sequence 08 out of it. Needs zstd
#    (the `zstandard` package, which `uv sync` installs, or the `zstd` binary).
#    Drop --include for the whole SCPNet group: 3.10 GiB / 3.33 GB down, 324 GiB / 348 GB
#    unpacked (see docs/DATASET.md). --include narrows the prediction/object-bank groups;
...
| Base-model predictions (SCPNet, JS3C-Net, LMSCNet) for val + test | Hugging Face [`Stone-Chern/GSSC-S2D2-datasets`](https://huggingface.co/datasets/Stone-Chern/GSSC-S2D2-datasets), as nine `zstd` archives that `scripts/download_assets.py` fetches **and unpacks** — or reproduce the JS3C-Net / LMSCNet sets locally via `scripts/dump_{js3c,lmscnet}_predictions.py`, see [docs/DATASET.md](docs/DATASET.md) | **5.26 GiB / 5.65 GB to download**, 558.0 GiB / 599.2 GB unpacked (SCPNet 3.10 GiB → 323.7 GiB, JS3C-Net 1.35 GiB → 189.0 GiB, LMSCNet 835 MiB → 45.3 GiB). The SCPNet figure materialises the three symlinked `synthetic*` farms; the 176.7 GiB / 189.7 GB a retrain actually reads is `..._sequences` + `..._synthetic`. The headline eval reads SCPNet val seq 08 alone — one 703 MiB / 737 MB archive, 8.4 GiB / 9.1 GB unpacked, fetchable on its own with `--include 'scpnet_predictions/08/*'` |
| Object bank (57,789 instances, 8 rare classes) | Hugging Face [`Stone-Chern/GSSC-S2D2-datasets`](https://huggingface.co/datasets/Stone-Chern/GSSC-S2D2-datasets), one `zstd` archive — see [docs/DATASET.md](docs/DATASET.md) | 46 MiB / 48 MB to download, 313 MiB / 328 MB of data unpacked |
| Synthetic pool (**two** variants released: 31K and 57K) | **Cite** the DOI, **download** from either host. Citable identifier: IEEE DataPort — [doi:10.21227/nqgf-9k39](https://dx.doi.org/10.21227/nqgf-9k39). Free download: the Hugging Face mirror [`Stone-Chern/PS3-SemanticKITTI`](https://huggingface.co/datasets/Stone-Chern/PS3-SemanticKITTI) (byte-identical archives, same CC-BY-NC-SA 4.0 LICENSE), which `scripts/download_assets.py --synthetic-pool {31K,57K}` fetches and which needs no IEEE subscription — it is public and serves anonymous downloads. *The DataPort record is the alternative for anyone who prefers the archival deposit, but it is not a click-and-download: it is marked "Subscription Required", serves files only to a signed-in session, and publishes no direct file URL. The local PS³ rebuild in [docs/DATASET.md](docs/DATASET.md) needs neither host.* | 127 GiB / 136 GB (31K), 229 GiB / 246 GB (57K), uncompressed; the archives themselves are 2.15 GiB / 2.31 GB (`synthetic_pool_31K.tar.gz`) and 3.88 GiB / 4.16 GB (`synthetic_pool_57K.tar.gz`) |

Sizes above were measured on the staged release payload — download figures from the uploaded archives, on-disk figures with `du -Lsb` (apparent size, symlinks dereferenced) — and are quoted as `GiB / GB` so the two conventions cannot be confused; [docs/DATASET.md](docs/DATASET.md) carries the same table per archive and per sequence. **The three prediction trees and the object bank are 609,349 files uncompressed, and several of their directories exceed the 10,000 files Hugging Face serves per directory, so they ship as ten `zstd` archives — nine holding the predictions, one the object bank — rather than as a per-frame tree**; `scripts/download_assets.py` downloads only the archives a request needs and extracts them into the `data/<prefix>/…` layout the docs describe, so no manual `tar` step is involved. Unpacking needs `zstd` (the `zstandard` package or the binary). The 0K, 10K and 20K data-scaling rows are **retrains, not released pools** — 0K means real frames only, and the 10K / 20K subsets are not staged for release.

These artefacts are released under two different terms. GSSC-authored code and the GSSC-trained model weights are MIT. The synthetic pool, the object bank and the base-model prediction dumps are derived from SemanticKITTI, which is distributed under CC-BY-NC-SA 4.0, so they inherit its non-commercial, share-alike restriction; the model weights, although MIT as our contribution, were also trained on SemanticKITTI, so downstream use of the weights still carries that non-commercial caveat. SemanticKITTI raw data follows its own license (see [semantic-kitti.org](http://www.semantic-kitti.org/)). `scripts/download_assets.py` populates every entry in the table, the synthetic pool included: `--synthetic-pool {31K,57K}` pulls the requested archive from the free Hugging Face mirror and then prints the IEEE DataPort DOI to cite it by. What it cannot do is fetch from DataPort itself — that record publishes no direct file URL and releases files only to a signed-in, subscribed session — so the mirror is the route the script takes. If a repo is unreachable the command exits with the manual-build instructions in `docs/DATASET.md`, which also documents manual provisioning for the rest.
...
Cite this DOI whichever host you took the bytes from. The archives are also mirrored, byte-identical, on Hugging Face at [`Stone-Chern/PS3-SemanticKITTI`](https://huggingface.co/datasets/Stone-Chern/PS3-SemanticKITTI) — that mirror is the free download route (public, no IEEE subscription, anonymous download) and is what `scripts/download_assets.py --synthetic-pool {31K,57K}` fetches, but it is not a citable identifier and does not replace the DOI above.

The submission snapshot referenced in the paper supplementary is the **v2.4.1** release; its Hydra configs hold the same hyperparameters listed in the paper's reproducibility appendix. Keep this tag in step with the one named in the paper's reproducibility appendix — the paper cites the tag by name, so bumping one without the other strands the reference.
...
* **Synthetic pool + object bank:** derived from SemanticKITTI and therefore distributed under the same CC-BY-NC-SA 4.0 (non-commercial, share-alike) terms; see [semantic-kitti.org/dataset.html](http://www.semantic-kitti.org/dataset.html). The pool is deposited on IEEE DataPort as [doi:10.21227/nqgf-9k39](https://dx.doi.org/10.21227/nqgf-9k39) — the identifier to cite — and mirrored for free download on Hugging Face at [`Stone-Chern/PS3-SemanticKITTI`](https://huggingface.co/datasets/Stone-Chern/PS3-SemanticKITTI); that redistribution is what the CC-BY-NC-SA 4.0 grant permits, and the mirrored archives ship the same LICENSE. It has its own citation (see [Citation](#citation) above).
