...
<sub>**Stage A — offline data augmentation (run once, pooled with real training).** Real SemanticKITTI GT scenes train pyramid multinomial diffusion $p_\theta\!:\,\mathcal{S}_1\!\to\!\mathcal{S}_2\!\to\!\mathcal{S}_3$ at $32^2{\times}4 \to 64^2{\times}8 \to 256^2{\times}32$. Stage 3 samples synthetic complete scenes $\hat{\mathbf{Y}}$; a Velodyne HDL-64E Bresenham3D ray-tracer ($64{\times}2048$ rays) converts each $\hat{\mathbf{Y}}$ into a matching sparse input $\hat{\mathbf{X}}$; a balanced 5,000-instance / 10-rare-class object bank ($500$ instances per class: bicycle, motorcycle, truck, other-vehicle, person, bicyclist, motorcyclist, pole, traffic-sign, trunk) pastes rare-class objects on ground-level voxels. The resulting synthetic (sparse, complete) pairs are pooled with the real SemanticKITTI training split, once, before S²D² training. **Stage B — S²D² deployment (one forward pass on any test scan).** A real sparse LiDAR scan is voxelized and passed through a frozen base SSC model $g_\phi$ (e.g., SCPNet, LMSCNet); a per-voxel argmax produces the one-hot source distribution $\mathbf{x}_{\text{src}} \in \Delta^{K-1}$, used as the source initialisation $\mathbf{x}_T = \mathbf{x}_{\text{src}}$. The denoising network $f_\theta$ — a sparse 3D U-Net (4 encoder/decoder stages, $32{\to}256$ channels) with FiLM$(t,\mathbf{L},\mathbf{B})$ modulation at every level — is conditioned on the base-derived BEV map $\mathbf{B}$ and the single-frame LiDAR voxel occupancy $\mathbf{L}$, and produces the refined output $\hat{\mathbf{x}}_0$ in one forward pass with EMA weights. No distillation. Source: paper Fig. 2.</sub>
...
<sub>**Top — intuition.** S²D² transports the base prediction $\mathbf{x}_{\text{src}}$ (gray cluster) to the ground truth $\mathbf{x}_0$ (orange cluster) in one Euler step on the per-voxel simplex; the learned velocity field $\mathbf{v}_\theta = f_\theta(\mathbf{x}_t,t,\mathbf{c}) - \mathbf{x}_{\text{src}}$, with $\mathbf{c}=\{\mathbf{B},\mathbf{L}\}$, gives the deployment update $\mathbf{x}_{t-1} = \mathbf{x}_t + \Delta\bar\alpha_t \cdot \mathbf{v}_\theta$. Richer $(t,\mathbf{L},\mathbf{B})$ conditioning sharpens the transport. **Middle — architecture.** $f_\theta$ is a 4-level sparse 3D U-Net at shapes $256^2{\times}32 \to 128^2{\times}16 \to 64^2{\times}8 \to 32^2{\times}4$ with channels $32 \to 256$, a $16^2{\times}2$ bottleneck containing two Sparse Residual Blocks, and a mirror decoder. Every E/D/bottleneck block is the same Sparse Residual Block (SubMConv3d → GN+SiLU → SubMConv3d → residual add → FiLM$(t,\mathbf{L},\mathbf{B})$); only the channel width $C$ varies. Three conditioning buses feed the FiLM modulators: timestep $t$ (sinMLP), multi-scale LiDAR features $\mathbf{L}$, and the base-derived BEV $\mathbf{B}$ (broadcast along $z$). **Bottom — qualitative example.** A rare-class win on SemanticKITTI val seq 08 frame 001390: base model motorcyclist IoU $27.3\%$, S²D² ($N{=}1$, single Euler step) $\mathbf{41.5\%}$, GT contains 322 motorcyclist voxels — a $+14.3$ point jump that is representative of the per-class gains reported below. Source: paper Fig. 3.</sub>
