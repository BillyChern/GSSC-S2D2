...
<sub>**Stage A** (top) — offline data augmentation: pyramid multinomial diffusion ($\mathcal{S}_1\!\to\!\mathcal{S}_2\!\to\!\mathcal{S}_3$ at $32^2{\times}4\to 64^2{\times}8\to 256^2{\times}32$) synthesises complete scenes; an HDL-64E Bresenham3D ray-tracer ($64{\times}2048$ rays) converts each into a matching sparse input; a balanced 5,000-instance / 10-rare-class object bank pastes rare classes on ground-level voxels. The synthetic pairs are pooled with the real SemanticKITTI training split. **Stage B** (bottom) — at deployment, a real sparse LiDAR scan is voxelized through a frozen base SSC model $g_\phi$ (e.g. SCPNet, LMSCNet) to produce $\mathbf{x}_{\text{src}}$, then refined by $f_\theta$ — a sparse 3D U-Net with FiLM$(t,\mathbf{L},\mathbf{B})$ at every level — into $\hat{\mathbf{x}}_0$ in one forward pass with EMA weights. No distillation. Source: paper Fig. 2.</sub>
...
<sub>**Top:** the learned velocity field $\mathbf{v}_\theta = f_\theta(\mathbf{x}_t,t,\mathbf{c}) - \mathbf{x}_{\text{src}}$ transports the base prediction (gray cluster) toward the ground truth (orange cluster) in a single Euler step on the per-voxel simplex. **Middle:** $f_\theta$ is a 4-level sparse 3D U-Net (stages $256^2{\times}32\to 32^2{\times}4$, channels $32\to 256$, $16^2{\times}2$ bottleneck with two Sparse Residual Blocks, mirror decoder) with FiLM$(t,\mathbf{L},\mathbf{B})$ modulation at every level and three conditioning buses (timestep $t$, multi-scale LiDAR $\mathbf{L}$, base-derived BEV $\mathbf{B}$). **Bottom:** a representative rare-class recovery on SemanticKITTI val seq 08 frame 001390 — motorcyclist IoU lifts from $27.3\%$ to $\mathbf{41.5\%}$ with a single Euler step ($N{=}1$), a $+14.3$ point jump. Source: paper Fig. 3.</sub>
